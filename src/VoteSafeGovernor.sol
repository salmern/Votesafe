    ...
    ReentrancyGuard
{
    ...
    event ProposalThresholdUpdated(
        ...
        uint256 oldEmergencyThreshold,
        uint256 newEmergencyThreshold
    );
    
    event EmergencyVotingPeriodUpdated(
        uint256 oldPeriod,
        uint256 newPeriod
    );
    ...
    function propose(
        ...
        if (options.length > 20) { // Reasonable limit
    ...
    function _createQuadraticProposal(
        uint256 proposalId,
        string memory description,
        string[] memory options
    ) internal {
    ...
    function updateProposalThresholds(
        uint256 _newProposalThresholdBPS,
        uint256 _newEmergencyProposalThresholdBPS
        ...
        emit ProposalThresholdUpdated(
            oldThreshold,
            _newProposalThresholdBPS,
            oldEmergencyThreshold,
            _newEmergencyProposalThresholdBPS
        );
    }
    
    function updateEmergencyVotingPeriod(
        uint256 _newEmergencyVotingPeriod
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function getQuadraticResult(uint256 proposalId) 
        external 
        view 
        returns (QuadraticResult memory) 
    {
